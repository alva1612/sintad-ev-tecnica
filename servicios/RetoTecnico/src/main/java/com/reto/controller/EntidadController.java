...
public class EntidadController {
	...
	public ResponseEntity<?> actualiza(@RequestBody EntidadDTO obj){
		...
		if (optAlumno.isPresent()) {
			...
			if (objSalida == null) {
				...
				salida.put("mensaje", "Se actualizó entidad: " + objSalida.getId());
			}
		}else {
	...
	@ResponseBody
	@CrossOrigin
	public ResponseEntity<?> buscarEntidad(@PathVariable("id") int id) {
		...
		if(obj.isPresent()) {
			...
			salida.put("mensaje", "Entidad no encontrada: " + id);
			return ResponseEntity.ok(salida);
		}
	}
